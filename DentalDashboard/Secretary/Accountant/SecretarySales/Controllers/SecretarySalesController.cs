...
namespace DentalDashboard.Secretary.Accountant.SecretarySales.Controllers;

[ApiController]
[Authorize(Roles = "Secretary")]
[Route("api/secretary/account/sales")]
public sealed class SecretarySalesController(ICommandDispatcher commands, IQueryDispatcher queries) : ControllerBase
{
    [HttpPost]
    ...
    public async Task<IActionResult> Services(CancellationToken cancellationToken) =>
        Ok(await queries.DispatchAsync(new GetActiveSecretarySaleServicesQuery(), cancellationToken));

    [HttpGet("patients")]
    public async Task<IActionResult> Patients([FromQuery] SearchSecretarySalePatientsQuery query, CancellationToken cancellationToken) =>
    ...
    {
        if (!TryGetCurrentUserId(out var secretaryUserId)) return Unauthorized();
        return Ok(await queries.DispatchAsync(new GetSecretaryWalletQuery { SecretaryUserId = secretaryUserId }, cancellationToken));
    }
