...
public sealed class GetPatientFinancialCasesQueryHandler(
  ...
  HandleAsync(GetPatientFinancialCasesQuery q, CancellationToken ct = default) {
    ...
    var count = await x.CountAsync(ct);var items=await x.OrderByDescending(a=>a.CreatedAt).Skip((p-1)*z).Take(z).Select(a=>new PatientFinancialCaseDto(a.Id,a.PatientId,a.Patient.UserId,(a.Patient.User.FirstName+" "+a.Patient.User.LastName).Trim(),a.Patient.User.PhoneNumber,(int)a.Service,a.Service.ToString(),a.TotalAmount,a.InitialPaymentAmount,a.InitialPaymentAmount+(a.Transactions.Sum(t=>(decimal?)t.Amount)??0),a.TotalAmount-a.InitialPaymentAmount-(a.Transactions.Sum(t=>(decimal?)t.Amount)??0),a.Debts.Where(d=>d.Status==PatientDebtStatus.Unpaid).Sum(d=>(decimal?)d.Amount)??0,a.AgreementType,a.Status,a.CreatedAt)).ToListAsync(ct);
